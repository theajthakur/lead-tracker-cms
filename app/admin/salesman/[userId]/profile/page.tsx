...
export default async function SalesmanProfilePage({ params }: PageProps) {
    ...
    if (analytics.status === "error" || !analytics.data) {
        return notFound();
    }
    ...
    return (
        <div className="flex flex-col gap-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    <h1 className="text-3xl font-bold tracking-tight">Salesman Profile</h1>
                ...
                <Button asChild>
                    <Link href={`./leads`}>
                        View Leads <ExternalLink className="ml-2 h-4 w-4" />
                    </Link>
                </Button>
            </div>
